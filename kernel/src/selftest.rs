...
const TESTS: &[(&str, TestFn)] = &[
    ...
    ("user_reset", test_user_reset),
    ("user_backup_agenda", test_user_backup_agenda),
...
/// Driver NVMe em ring 3 servindo o `nexo.block`: o MESMO cliente cru do blockdev (modo 8)
/// roda contra o `nvmedev` — capacidade, escrita/leitura com padrão e marcador de persistência
/// no disco NVMe de dados. Substituibilidade por protocolo, com um controlador PCI real (QEMU).
/// Driver AHCI em ring 3 servindo `nexo.block`: o MESMO cliente cru (modo 8) roda contra o
/// `ahcidev` — terceiro controlador de armazenamento sob o mesmo protocolo.
fn test_user_ahci() -> TestResult {
    use crate::ipc::{ChannelEnd, DeviceGrant, Handle, Object, Rights};
...
fn test_user_ahci_fs() -> TestResult {
    use crate::ipc::{ChannelEnd, DeviceGrant, Handle, Object, Rights};
    let Some(bdf) = crate::pci::devices()
        .iter()
        .find(|d| d.class == 0x01 && d.subclass == 0x06 && d.prog_if == 0x01)
...
/// Primeiro utilitario POSIX portado (`wc`) com a convencao de argv do Nexo: o criador entrega
/// um canal no handle 1 com UMA mensagem ja enviada — os argumentos separados por `\0` (argv[0]
/// incluso) — e o crt0 da nexo-libc monta `main(argc, argv)` a partir dela. O `wc-c` abre o
/// arquivo que `user_c_fs` acabou de escrever (mesmo boot, registrado antes) e loga
/// "0 5 26 /c-arquivo.txt"; aqui conferimos a saida 0 e os vazamentos. Guardado pela presenca.
fn test_user_wc() -> TestResult {
    use crate::ipc::{ChannelEnd, Message};
    if crate::initrd::find("wc-c").is_none() {
        return Err(String::from(
            "wc-c ausente do initrd (host sem clang/rust-lld)",
        ));
    }
    if !has_virtio_blk() {
        return Err(String::from(
    ...
    argv_tx
        .send(Message {
            data: b"wc\0/c-arquivo.txt\0".to_vec(),
            handles: alloc::vec![],
        })
...
/// Health check pós-boot do layout A/B (ADR-0010): o `upd`, com o canal `nexo.block` do DISCO
/// DE BOOT real (servido pelo `ahcidev` restrito ao SATA integrado 00:1f.2, onde o QEMU anexa
/// a imagem), localiza `\nexo\slots.bin` (GPT → FAT), marca o slot arrancado como saudável e o
/// driver (modo 67) confere pelo "estado" — que RELÊ o setor do disco — s=1 e tentativas
/// repostas. É a contraparte do desconto de tentativas do loader (rollback automático:
/// `tools/test-rollback`).
fn test_user_slots_confirm() -> TestResult {
    use crate::ipc::{ChannelEnd, DeviceGrant, Handle, Object, Rights};
    // O disco de boot fica no SATA integrado do q35, sempre em 00:1f.2 (bdf 0xfa).
    if !crate::pci::devices()
        .iter()
...
/// Agendamento do backup: o driver (modo 70) grava dados, entrega a capacidade do fs por
/// **contrato** ("agenda 1 s") — retida pela duração, não por pedido —, o serviço espelha
/// sozinho a cada intervalo, e "cancela" a devolve com a contagem (≥ 2 em ~2,5 s). Um
/// desastre local é então revertido a partir do espelho AGENDADO. Mesma pilha dupla do backup.
fn test_user_backup_agenda() -> TestResult {
    use crate::ipc::{ChannelEnd, DeviceGrant, Handle, Object, Rights};
    if !has_virtio_blk() {
        return Err(String::from(
            "virtio-blk ausente (rode com o disco de dados)",
    ...
    drop((driver, bkp, fs1, fs2, blk, ahcid));
    sched::reap();
...
/// Reset preservando arquivos: o driver (modo 69) cria dados de usuário (/home) e "estado de
/// sistema" (/cfg, /lixo aninhado) no volume de dados, espelha /home no disco de backup
/// ("quando possível"), e o `reset` limpa o volume em pós-ordem preservando /home — o driver
/// confere que os dados do usuário ficaram e o resto sumiu. Mesma pilha dupla do backup
/// (blockdev+fs principal, ahcidev+fs de backup) + os serviços `backup` e `reset`.
fn test_user_reset() -> TestResult {
    use crate::ipc::{ChannelEnd, DeviceGrant, Handle, Object, Rights};
    if !has_virtio_blk() {
        return Err(String::from(
            "virtio-blk ausente (rode com o disco de dados)",
    ...
    let fs2 =
        crate::process::spawn_named("fs", 0, alloc::vec![channel_handle(f), channel_handle(g)])
            .map_err(String::from)?;
    // serviços backup + reset e o driver
    let (pa, pb) = ChannelEnd::create_pair();
    let bkp = crate::process::spawn_named(
        "backup",
        0,
    ...
    .map_err(String::from)?;
    let (ra, rb) = ChannelEnd::create_pair();
    let rst = crate::process::spawn_named("reset", 0, alloc::vec![channel_handle(ra)])
        .map_err(String::from)?;
    let driver = crate::process::spawn_named(
        "utest",
        69,
        alloc::vec![channel_handle(pb), channel_handle(rb), channel_handle(d)],
    )
    .map_err(String::from)?;
    let cc = crate::process::wait_and_reap(&driver);
    let kc = crate::process::wait_and_reap(&bkp);
    let rc = crate::process::wait_and_reap(&rst);
    let f1 = crate::process::wait_and_reap(&fs1);
    let f2 = crate::process::wait_and_reap(&fs2);
    let bc = crate::process::wait_and_reap(&blk);
    let ac = crate::process::wait_and_reap(&ahcid);
    drop((driver, bkp, rst, fs1, fs2, blk, ahcid));
    sched::reap();
    check!(cc == 0, "driver saiu com {cc}");
    check!(kc == 0, "backup saiu com {kc}");
    check!(rc == 0, "reset saiu com {rc}");
    check!(f1 == 0, "fs principal saiu com {f1}");
    check!(f2 == 0, "fs de backup saiu com {f2}");
    check!(bc == 0, "blockdev saiu com {bc}");
    check!(ac == 0, "ahcidev saiu com {ac}");
...
fn test_user_portal() -> TestResult {
    use crate::ipc::{ChannelEnd, Handle, Object, Rights};
    if !has_virtio_blk() {
        return Err(String::from(
            "virtio-blk ausente (rode com o disco de dados)",
    ...
    let fs =
        crate::process::spawn_named("fs", 0, alloc::vec![channel_handle(b), channel_handle(c)])
            .map_err(String::from)?;
    let portal = crate::process::spawn_named("portal", 0, alloc::vec![channel_handle(pa)])
        .map_err(String::from)?;
    let driver = crate::process::spawn_named(
        "utest",
        60,
    ...
    .map_err(String::from)?;
    let dc = crate::process::wait_and_reap(&driver);
    let vc = crate::process::wait_and_reap(&portal);
    let fc = crate::process::wait_and_reap(&fs);
    let bc = crate::process::wait_and_reap(&blk);
    let wc = crate::process::wait_and_reap(&wm);
    drop((wm, blk, fs, portal, driver));
    sched::reap();
    let frames = settled_free_frames(frames0, 8);
    check!(dc == 0, "driver saiu com {dc}");
    check!(vc == 0, "portal saiu com {vc}");
    check!(fc == 0, "servidor fs saiu com {fc}");
    check!(bc == 0, "blockdev saiu com {bc}");
    check!(wc == 0, "wm saiu com {wc}");
    let ends = crate::ipc::live_channel_ends();
