...
fn repo_net_client(http_port: u16) -> ! {
    ...
    let v = nexo_inst::install_from_repo(&mut fs, "app-net", buf).unwrap_or_else(|e| {
        nexo_rt::log!("utest: repo-net: instalacao falhou: {:?}", e);
        nexo_sys::exit(2619)
    });
    if v == 0 {
        nexo_sys::exit(2620);
...
/// 16 da página da pilha e 16 da página do primeiro mapeamento; o kernel compara duas
/// execuções.
fn aslr_probe() -> ! {
    let marker = 0u8;
    let sp = core::ptr::addr_of!(marker) as u64;
    let text = aslr_probe as *const () as usize as u64;
    ...
    let code =
        (((text >> 12) & 0xffff) << 32) | (((sp >> 12) & 0xffff) << 16) | ((base >> 12) & 0xffff);
