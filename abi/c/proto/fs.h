/* nexo_proto_fs.h — protocolo tipado `nexo.fs` v1.4 em C.
...
/* Codifica o pedido `rename` (metodo 11); devolve o tamanho ou -1. */
static inline int nexo_fs_rename_req_encode(uint8_t *out, size_t cap, const nexo_fs_rename_req *m) {
    size_t o = 24;
    if (m->from_len > 256 || o + 4 + m->from_len > cap) return -1;
    { uint32_t l = m->from_len; size_t i; for (i = 0; i < 4; i++) out[o + i] = (uint8_t)(l >> (8 * i)); }
    { size_t i; for (i = 0; i < m->from_len; i++) out[o + 4 + i] = m->from[i]; }
    o += 4 + m->from_len;
    if (m->to_len > 256 || o + 4 + m->to_len > cap) return -1;
    { uint32_t l = m->to_len; size_t i; for (i = 0; i < 4; i++) out[o + i] = (uint8_t)(l >> (8 * i)); }
    { size_t i; for (i = 0; i < m->to_len; i++) out[o + 4 + i] = m->to[i]; }
    o += 4 + m->to_len;
    if (cap < 24) return -1;
    /* magic NXIP: u32 0x4e584950 em little-endian no fio */
    out[0] = 0x50; out[1] = 0x49; out[2] = 0x58; out[3] = 0x4e;
...
/* Decodifica a resposta de `rename`: 0 = ok; >0 = erro remoto; -1 = malformada. */
static inline int nexo_fs_rename_resp_decode(const uint8_t *b, size_t len, nexo_fs_rename_resp *m) {
    if (len < 24 || b[0] != 0x50 || b[1] != 0x49 || b[2] != 0x58 || b[3] != 0x4e) return -1;
    { uint32_t fl = (uint32_t)b[16] | ((uint32_t)b[17] << 8) | ((uint32_t)b[18] << 16) | ((uint32_t)b[19] << 24);
      if (fl & 2u) { if (len < 28) return -1;
        return (int)((uint32_t)b[24] | ((uint32_t)b[25] << 8) | ((uint32_t)b[26] << 16) | ((uint32_t)b[27] << 24)); } }
...
typedef struct {
    ...
    uint32_t duplicadas;
    uint32_t orfaos;
    uint32_t blocos_duplicados;
    uint32_t fora_da_faixa;
    uint32_t bitmap;
} nexo_fs_check_resp;

/* Codifica o pedido `check` (metodo 13); devolve o tamanho ou -1. */
static inline int nexo_fs_check_req_encode(uint8_t *out, size_t cap, const nexo_fs_check_req *m) {
    size_t o = 24;
    (void)m;
    if (cap < 24) return -1;
    /* magic NXIP: u32 0x4e584950 em little-endian no fio */
...
/* Decodifica a resposta de `check`: 0 = ok; >0 = erro remoto; -1 = malformada. */
static inline int nexo_fs_check_resp_decode(const uint8_t *b, size_t len, nexo_fs_check_resp *m) {
    if (len < 24 || b[0] != 0x50 || b[1] != 0x49 || b[2] != 0x58 || b[3] != 0x4e) return -1;
    { uint32_t fl = (uint32_t)b[16] | ((uint32_t)b[17] << 8) | ((uint32_t)b[18] << 16) | ((uint32_t)b[19] << 24);
      if (fl & 2u) { if (len < 28) return -1;
        return (int)((uint32_t)b[24] | ((uint32_t)b[25] << 8) | ((uint32_t)b[26] << 16) | ((uint32_t)b[27] << 24)); } }
    { size_t o = 24;
      { uint64_t v = 0; size_t i; if (o + 4 > len) return -1; for (i = 0; i < 4; i++) v |= (uint64_t)b[o + i] << (8 * i); m->inodes = (uint32_t)v; o += 4; }
      { uint64_t v = 0; size_t i; if (o + 4 > len) return -1; for (i = 0; i < 4; i++) v |= (uint64_t)b[o + i] << (8 * i); m->blocos = (uint32_t)v; o += 4; }
      { uint64_t v = 0; size_t i; if (o + 4 > len) return -1; for (i = 0; i < 4; i++) v |= (uint64_t)b[o + i] << (8 * i); m->penduradas = (uint32_t)v; o += 4; }
      { uint64_t v = 0; size_t i; if (o + 4 > len) return -1; for (i = 0; i < 4; i++) v |= (uint64_t)b[o + i] << (8 * i); m->duplicadas = (uint32_t)v; o += 4; }
      { uint64_t v = 0; size_t i; if (o + 4 > len) return -1; for (i = 0; i < 4; i++) v |= (uint64_t)b[o + i] << (8 * i); m->orfaos = (uint32_t)v; o += 4; }
      { uint64_t v = 0; size_t i; if (o + 4 > len) return -1; for (i = 0; i < 4; i++) v |= (uint64_t)b[o + i] << (8 * i); m->blocos_duplicados = (uint32_t)v; o += 4; }
      { uint64_t v = 0; size_t i; if (o + 4 > len) return -1; for (i = 0; i < 4; i++) v |= (uint64_t)b[o + i] << (8 * i); m->fora_da_faixa = (uint32_t)v; o += 4; }
      { uint64_t v = 0; size_t i; if (o + 4 > len) return -1; for (i = 0; i < 4; i++) v |= (uint64_t)b[o + i] << (8 * i); m->bitmap = (uint32_t)v; o += 4; }
    }
    return 0;
}
...
/* Codifica o pedido `open` (metodo 14); devolve o tamanho ou -1. */
static inline int nexo_fs_open_req_encode(uint8_t *out, size_t cap, const nexo_fs_open_req *m) {
    size_t o = 24;
    if (o + 8 > cap) return -1;
    { uint64_t v = (uint64_t)m->mounts; size_t i; for (i = 0; i < 8; i++) out[o + i] = (uint8_t)(v >> (8 * i)); }
    o += 8;
    if (cap < 24) return -1;
    /* magic NXIP: u32 0x4e584950 em little-endian no fio */
    out[0] = 0x50; out[1] = 0x49; out[2] = 0x58; out[3] = 0x4e;
...
static inline int nexo_fs_open_resp_decode(const uint8_t *b, size_t len, nexo_fs_open_resp *m) {
    if (len < 24 || b[0] != 0x50 || b[1] != 0x49 || b[2] != 0x58 || b[3] != 0x4e) return -1;
    { uint32_t fl = (uint32_t)b[16] | ((uint32_t)b[17] << 8) | ((uint32_t)b[18] << 16) | ((uint32_t)b[19] << 24);
      if (fl & 2u) { if (len < 28) return -1;
        return (int)((uint32_t)b[24] | ((uint32_t)b[25] << 8) | ((uint32_t)b[26] << 16) | ((uint32_t)b[27] << 24)); } }
