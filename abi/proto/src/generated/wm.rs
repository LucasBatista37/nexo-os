...
impl DragStartRequest {
    ...
    pub fn decode_payload(b: &[u8]) -> Result<Self, ProtoError> {
        ...
        let _ = o;
        Ok(DragStartRequest { data, data_len })
    }
    /// Codifica a mensagem completa (cabecalho NXIP + payload).
    pub fn encode_msg(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
        if out.len() < HEADER_LEN {
...
/// `nexo.wm.drag_start` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct DragStartResponse {}

impl DragStartResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 28;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
...
/// `nexo.wm.surface_info` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SurfaceInfoResponse {
    /// Campo `used`.
    pub used: u8,
    /// Campo `id`.
    pub id: u32,
...
impl SurfaceInfoResponse {
    ...
    pub fn encode_payload(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
        ...
        out[o..o + 1].copy_from_slice(&self.used.to_le_bytes());
        o += 1;
        if o + 4 > out.len() {
            return Err(ProtoError::Short);
        }
    ...
    pub fn decode_payload(b: &[u8]) -> Result<Self, ProtoError> {
        ...
        let used = u8::from_le_bytes(b[o..o + 1].try_into().unwrap());
        o += 1;
        if o + 4 > b.len() {
            return Err(ProtoError::Short);
        }
...
impl SetScaleRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 35;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
    /// Codifica o payload; devolve o tamanho.
    pub fn encode_payload(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
...
/// `nexo.wm.set_scale` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SetScaleResponse {}

impl SetScaleResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 35;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
...
/// `nexo.wm.set_reduce_motion` — pedido.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SetReduceMotionRequest {
    /// Campo `enabled`.
    pub enabled: u8,
}

impl SetReduceMotionRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 36;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
    /// Codifica o payload; devolve o tamanho.
    pub fn encode_payload(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
    ...
    pub fn decode_payload(b: &[u8]) -> Result<Self, ProtoError> {
        ...
        o += 1;
        let _ = o;
        Ok(SetReduceMotionRequest { enabled })
    }
    /// Codifica a mensagem completa (cabecalho NXIP + payload).
    pub fn encode_msg(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
        if out.len() < HEADER_LEN {
...
/// `nexo.wm.set_reduce_motion` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SetReduceMotionResponse {}

impl SetReduceMotionResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 36;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
...
/// `nexo.wm.prefs` — pedido.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PrefsRequest {}

impl PrefsRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 37;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
    /// Codifica o payload; devolve o tamanho.
    pub fn encode_payload(&self, _out: &mut [u8]) -> Result<usize, ProtoError> {
...
/// `nexo.wm.set_auto_tile` — pedido.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SetAutoTileRequest {
    /// Campo `enabled`.
    pub enabled: u8,
}

impl SetAutoTileRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 42;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
    /// Codifica o payload; devolve o tamanho.
    pub fn encode_payload(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
...
/// `nexo.wm.set_auto_tile` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SetAutoTileResponse {}

impl SetAutoTileResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 42;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
...
/// `nexo.wm.notification_info` — pedido.
#[derive(Clone, Debug, PartialEq, Eq)]
...
impl NotificationInfoRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 38;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
    /// Codifica o payload; devolve o tamanho.
    pub fn encode_payload(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
    ...
    pub fn decode_payload(b: &[u8]) -> Result<Self, ProtoError> {
        ...
        o += 4;
        let _ = o;
        Ok(NotificationInfoRequest { index })
    }
    /// Codifica a mensagem completa (cabecalho NXIP + payload).
    pub fn encode_msg(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
        if out.len() < HEADER_LEN {
...
/// `nexo.wm.notification_info` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// `nexo.wm.notifications_clear` — pedido.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct NotificationsClearRequest {}

impl NotificationsClearRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 39;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
    /// Codifica o payload; devolve o tamanho.
    pub fn encode_payload(&self, _out: &mut [u8]) -> Result<usize, ProtoError> {
...
/// `nexo.wm.notifications_clear` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct NotificationsClearResponse {}

impl NotificationsClearResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 39;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
...
/// `nexo.wm.set_title` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SetTitleResponse {}

impl SetTitleResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 30;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
...
/// `nexo.wm.a11y_subscribe` — pedido.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct A11ySubscribeRequest {
    /// Handle `chan` (viaja no vetor de handles, nunca no payload).
    pub chan: u32,
}

impl A11ySubscribeRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 31;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 1;
    /// Handles na ordem de declaracao (para passar ao `channel_send`).
    pub fn handles(&self) -> [u32; 1] {
...
/// `nexo.wm.a11y_subscribe` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct A11ySubscribeResponse {}

impl A11ySubscribeResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 31;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
...
/// `nexo.wm.prefs_save` — pedido.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PrefsSaveRequest {
    /// Handle `fs` (viaja no vetor de handles, nunca no payload).
    pub fs: u32,
}

impl PrefsSaveRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 46;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 1;
    /// Handles na ordem de declaracao (para passar ao `channel_send`).
    pub fn handles(&self) -> [u32; 1] {
    ...
    /// ausentes assumem o padrao — ipc-compat §3).
    pub fn decode_payload(_b: &[u8]) -> Result<Self, ProtoError> {
        let fs: u32 = 0; // injetado por decode_*_with_handles
        Ok(PrefsSaveRequest { fs })
    }
    /// Codifica a mensagem completa (cabecalho NXIP + payload).
    pub fn encode_msg(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
        if out.len() < HEADER_LEN {
...
/// `nexo.wm.prefs_save` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PrefsSaveResponse {
    /// Handle `fs` (viaja no vetor de handles, nunca no payload).
    pub fs: u32,
}

impl PrefsSaveResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 46;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 1;
...
/// `nexo.wm.prefs_load` — pedido.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PrefsLoadRequest {
    /// Handle `fs` (viaja no vetor de handles, nunca no payload).
    pub fs: u32,
}

impl PrefsLoadRequest {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 47;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 1;
    /// Handles na ordem de declaracao (para passar ao `channel_send`).
    pub fn handles(&self) -> [u32; 1] {
...
/// `nexo.wm.prefs_load` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PrefsLoadResponse {
    /// Handle `fs` (viaja no vetor de handles, nunca no payload).
    pub fs: u32,
}

impl PrefsLoadResponse {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 47;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 1;
...
impl A11yEvent {
    /// Numero do metodo.
    pub const METHOD_ID: u32 = 32;
    /// Handles que esta mensagem carrega no vetor de handles da mensagem.
    pub const HANDLE_COUNT: usize = 0;
    /// Fatia valida de `text`.
    pub fn text(&self) -> &[u8] {
        &self.text[..(self.text_len as usize).min(32)]
    }
    /// Codifica o payload; devolve o tamanho.
    pub fn encode_payload(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
        let mut o = 0usize;
...
pub enum Request {
    ...
    /// `drag_start`.
    DragStart(DragStartRequest),
    /// `surface_info`.
    SurfaceInfo(SurfaceInfoRequest),
    /// `activate`.
    ...
    /// `notifications_clear`.
    NotificationsClear(NotificationsClearRequest),
    /// `set_title`.
    ...
    /// `prefs_save`.
    PrefsSave(PrefsSaveRequest),
    /// `prefs_load`.
    PrefsLoad(PrefsLoadRequest),
}
...
pub fn decode_request_with_handles(msg: &[u8], hs: &[u32]) -> Result<Request, ProtoError> {
    ...
    match &mut r {
        ...
        Request::PrefsSave(rq) => {
            if hs.len() != 1 {
                return Err(ProtoError::Length);
            }
            rq.fs = hs[0];
        }
        Request::PrefsLoad(rq) => {
            if hs.len() != 1 {
                return Err(ProtoError::Length);
            }
            rq.fs = hs[0];
...
pub fn decode_request(msg: &[u8]) -> Result<Request, ProtoError> {
    ...
    match h.method_id {
        ...
        28 => Ok(Request::DragStart(DragStartRequest::decode_payload(p)?)),
        33 => Ok(Request::SurfaceInfo(SurfaceInfoRequest::decode_payload(p)?)),
        34 => Ok(Request::Activate(ActivateRequest::decode_payload(p)?)),
        35 => Ok(Request::SetScale(SetScaleRequest::decode_payload(p)?)),
        36 => Ok(Request::SetReduceMotion(
        ...
        39 => Ok(Request::NotificationsClear(
            NotificationsClearRequest::decode_payload(p)?,
        )),
        30 => Ok(Request::SetTitle(SetTitleRequest::decode_payload(p)?)),
        ...
        46 => Ok(Request::PrefsSave(PrefsSaveRequest::decode_payload(p)?)),
        47 => Ok(Request::PrefsLoad(PrefsLoadRequest::decode_payload(p)?)),
...
/// Decodifica a resposta de `drag_start` (erro remoto vira `ProtoError::Remote`).
pub fn decode_drag_start_response(msg: &[u8]) -> Result<DragStartResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    DragStartResponse::decode_payload(p)
}

/// Decodifica a resposta de `surface_info` (erro remoto vira `ProtoError::Remote`).
pub fn decode_surface_info_response(msg: &[u8]) -> Result<SurfaceInfoResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    SurfaceInfoResponse::decode_payload(p)
}

/// Decodifica a resposta de `activate` (erro remoto vira `ProtoError::Remote`).
pub fn decode_activate_response(msg: &[u8]) -> Result<ActivateResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    ActivateResponse::decode_payload(p)
}

/// Decodifica a resposta de `set_scale` (erro remoto vira `ProtoError::Remote`).
pub fn decode_set_scale_response(msg: &[u8]) -> Result<SetScaleResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    SetScaleResponse::decode_payload(p)
}

/// Decodifica a resposta de `set_reduce_motion` (erro remoto vira `ProtoError::Remote`).
pub fn decode_set_reduce_motion_response(
    msg: &[u8],
) -> Result<SetReduceMotionResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    SetReduceMotionResponse::decode_payload(p)
}

/// Decodifica a resposta de `prefs` (erro remoto vira `ProtoError::Remote`).
pub fn decode_prefs_response(msg: &[u8]) -> Result<PrefsResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
...
pub fn decode_set_auto_tile_response(msg: &[u8]) -> Result<SetAutoTileResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    SetAutoTileResponse::decode_payload(p)
}
...
pub fn decode_notification_info_response(
    msg: &[u8],
) -> Result<NotificationInfoResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    NotificationInfoResponse::decode_payload(p)
}

/// Decodifica a resposta de `notifications_clear` (erro remoto vira `ProtoError::Remote`).
pub fn decode_notifications_clear_response(
    msg: &[u8],
) -> Result<NotificationsClearResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    NotificationsClearResponse::decode_payload(p)
}

/// Decodifica a resposta de `set_title` (erro remoto vira `ProtoError::Remote`).
pub fn decode_set_title_response(msg: &[u8]) -> Result<SetTitleResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    SetTitleResponse::decode_payload(p)
}

/// Decodifica a resposta de `a11y_subscribe` (erro remoto vira `ProtoError::Remote`).
pub fn decode_a11y_subscribe_response(msg: &[u8]) -> Result<A11ySubscribeResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
...
/// Decodifica a resposta de `prefs_save` (erro remoto vira `ProtoError::Remote`).
pub fn decode_prefs_save_response(msg: &[u8]) -> Result<PrefsSaveResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    PrefsSaveResponse::decode_payload(p)
}

/// Decodifica a resposta de `prefs_load` (erro remoto vira `ProtoError::Remote`).
pub fn decode_prefs_load_response(msg: &[u8]) -> Result<PrefsLoadResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    PrefsLoadResponse::decode_payload(p)
}

/// Decodifica um evento `pointer` (mensagem sem resposta).
pub fn decode_pointer_event(msg: &[u8]) -> Result<PointerEvent, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    let p = &msg[HEADER_LEN..HEADER_LEN + h.payload_len as usize];
    PointerEvent::decode_payload(p)
}

/// Decodifica um evento `drop` (mensagem sem resposta).
pub fn decode_drop_event(msg: &[u8]) -> Result<DropEvent, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
    ...
    let p = &msg[HEADER_LEN..HEADER_LEN + h.payload_len as usize];
    DropEvent::decode_payload(p)
}

/// Decodifica um evento `a11y` (mensagem sem resposta).
pub fn decode_a11y_event(msg: &[u8]) -> Result<A11yEvent, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
