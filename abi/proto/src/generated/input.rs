...
/// FNV-1a de `"nexo.input"`.
pub const PROTOCOL_ID: u32 = 0x41ccd08b;
...
/// `nexo.input.poll` — resposta.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PollResponse {
    /// Bytes de `events` (ate 3500).
    pub events: [u8; 3500],
    /// Tamanho valido de `events`.
    pub events_len: u32,
}
...
impl PollResponse {
    ...
    pub fn encode_payload(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
        let mut o = 0usize;
        let n = self.events_len as usize;
        if n > 3500 {
        ...
        out[o + 4..o + 4 + n].copy_from_slice(&self.events[..n]);
        o += 4 + n;
        Ok(o)
    }
    /// Decodifica o payload (bytes extras ao final sao ignorados; campos com padrao
    ...
    pub fn decode_payload(b: &[u8]) -> Result<Self, ProtoError> {
        ...
        let events_len: u32;
        {
            ...
            events[..l].copy_from_slice(&b[o + 4..o + 4 + l]);
            events_len = l as u32;
            o += 4 + l;
        }
        let _ = o;
        Ok(PollResponse { events, events_len })
    }
    /// Codifica a mensagem completa (cabecalho NXIP + payload).
    pub fn encode_msg(&self, out: &mut [u8]) -> Result<usize, ProtoError> {
        if out.len() < HEADER_LEN {
...
pub fn decode_request(msg: &[u8]) -> Result<Request, ProtoError> {
    ...
    match h.method_id {
        1 => Ok(Request::Poll(PollRequest::decode_payload(p)?)),
        2 => Ok(Request::Subscribe(SubscribeRequest::decode_payload(p)?)),
        _ => Err(ProtoError::Method),
    }
}

/// Decodifica a resposta de `poll` (erro remoto vira `ProtoError::Remote`).
pub fn decode_poll_response(msg: &[u8]) -> Result<PollResponse, ProtoError> {
    let h = Header::decode(msg)?;
    if h.protocol_id != PROTOCOL_ID {
        return Err(ProtoError::Protocol);
    }
