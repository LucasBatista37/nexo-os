...
- **Modo usuário**: ABI de syscalls v0 (`syscall`/`sysret`), processos com espaço de endereçamento próprio carregados de ELF, `services/init` rodando em ring 3, falhas de usuário isoladas do kernel.
- 30 auto-testes executados no boot e verificados por serial no CI; cenários de falha (panic, page fault, estouro de pilha) com diagnóstico útil; cenário de stress.
