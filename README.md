...
- **Dispositivos (Fase 3, início)**: enumeração PCI, concessões de dispositivo por handle (config PCI, MMIO restrito a BARs, DMA, vetores MSI) e o primeiro driver isolado em modo usuário — VirtIO-block com MSI-X (`services/blockdev`) — com teste de persistência entre boots (`tools/test-qemu --scenario storage`).
