...
- **Dispositivos e armazenamento (Fase 3, em andamento)**: enumeração PCI, concessões de dispositivo por handle (config PCI, MMIO restrito a BARs, DMA, vetores MSI), driver VirtIO-block isolado em modo usuário com MSI-X (`services/blockdev`) e o sistema de arquivos NexoFS v0 (`libraries/nexofs`, `services/fs`): arquivos criados, alterados e removidos sobrevivem a reboots (`tools/test-qemu --scenario storage`), cortes de energia simulados em cada escrita no host, driver que cai não derruba o kernel.
