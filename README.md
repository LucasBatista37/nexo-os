...
- **Modo usuário**: ABI de syscalls v0 (`syscall`/`sysret`), processos com espaço de endereçamento próprio carregados de um initramfs próprio, handles com direitos, canais IPC com transferência de handles, `init` + `svcmgr` que reinicia um serviço que cai (`echo`) sem reiniciar o kernel, falhas de usuário isoladas.
- **Dispositivos e armazenamento (Fase 3, em andamento)**: enumeração PCI, concessões de dispositivo por handle (config PCI, MMIO restrito a BARs, DMA, vetores MSI), gerenciador de dispositivos (`services/devmgr`) que entrega a cada driver uma concessão restrita ao seu dispositivo, transporte VirtIO compartilhado (`libraries/virtio`), drivers VirtIO-block e VirtIO-RNG isolados em modo usuário com MSI-X (`services/blockdev`, `services/rngdev`) o leitor FAT somente leitura da partição EFI (`libraries/fat`, `services/espfs`) o sistema de arquivos NexoFS v0 (`libraries/nexofs`, `services/fs`), um VFS com namespaces por processo (`services/vfs`: `/disk`, `/boot` só leitura, `/tmp` em ramfs) drivers VirtIO de entrada (`services/inputdev`, testado com teclas injetadas via QMP) e um shell de diagnóstico na console VirtIO (`services/shell` + `services/consoledev`; `tools/run-qemu --console-socket` e cenário `shell`): arquivos criados, alterados e removidos sobrevivem a reboots (`tools/test-qemu --scenario storage`), cortes de energia simulados em cada escrita no host, driver que cai não derruba o kernel.
- 32 auto-testes executados no boot e verificados por serial no CI; cenários de falha (panic, page fault, estouro de pilha) com diagnóstico útil; cenário de stress.
