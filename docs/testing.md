...
| Rede (QEMU) | `tools/test-qemu --scenario net` | `run-qemu --net` (slirp): DHCP completo (lease com máscara/gw/DNS), ARP no gateway do lease, ICMP echo reply conferido e uma consulta DNS A real por `example.com` ao servidor do lease (resposta com id/rcode validados); parsers com fuzz-lite no host (`cargo test -p nexo-netstack`) |
