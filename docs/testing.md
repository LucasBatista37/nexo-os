...
| Compositor (host) | `cargo test -p nexo-wm` | composição por z-order (estável), dano restringindo o repintar, acumulação/coalescência de danos |
| Renderizador 2D (host) | `cargo test -p nexo-gfx` | preenchimento, ordem de bytes por formato, composição alfa src-over, clipping, stride, blit entre superfícies; rasterização de texto (dimensões, fundo, quebra de linha, glifo de fallback) |
...
| Captura autorizada | `make netcap` (ou `tools/netcap --summary-only ARQ.pcap`) | sobe o Nexo com slirp, grava um pcap de todos os pacotes da interface (`--net-dump`) e resume por protocolo/fluxo; captura sempre local ao QEMU do usuário e explicitamente pedida |
| **Sessão gráfica** | `tools/test-qemu --scenario desktop` | o mesmo que `make run` faz, headless, em **dois boots no mesmo disco** (o 1.º com disco novo = primeira execução: o greeter pergunta o idioma e o host responde `2` por QMP; o 2.º carrega `en-US` das preferências e a pergunta é marcador proibido): boot `selftest=0 desktop=1` com teclado e ponteiro virtio, disco e tela; o kernel só concede dispositivos (inputdev ×2, devmgr, vídeo) e o `services/sessao` compõe o resto em modo usuário — `wm` a apresentar no framebuffer, vfs com /disk e /boot, Faixa de Atividades, greeter (o host digita uma senha errada e depois `nexo` por QMP), terminal com o shell; depois **Meta+L** (acorde por QMP) bloqueia a sessão a pedido e a senha desbloqueia-a de novo. A sessão **não termina** (é esse o ponto): o host mata o QEMU 0–500 ms depois de `sessao: desbloqueada (a pedido)` (código 99) e verifica os marcadores; `NEXO: boot completo` é **proibido** aqui — a sessão viva nunca chega lá |
| Rede (QEMU) | `tools/test-qemu --scenario net` | `run-qemu --net` (slirp), duas fases num boot: (1) caminho cru — DHCP, ARP, ICMP echo, DNS e handshake TCP com RST; (2) `netd` — API de sockets (`nexo.sock`): info do lease, DNS com cache (2ª consulta `cached=1`), eco UDP, TCP conectar/enviar/receber/fechar contra servidores reais no host uma **conexão de entrada** aceita pelo `netd` (`tcp_listen` + `--net-hostfwd`) e um GET HTTP/1.0 com resposta 200 validada; e, na fase 2, o firewall e os **sockets por sessão** (a filha restrita não usa a conexão nem a porta UDP do pai, a porta do DNS é reservada, e a conexão da filha é libertada quando ela fecha — o `netd` regista `sessao N fechou; … libertados`); (3) repositório em rede; (4) `fetch` em C; (5) **diagnóstico** (`nexo.sock` v1.1: `ping` ao gateway, `ping` a 192.0.2.1 com prazo esgotado, `stats`) — **última de propósito**: um ICMP encaminhado pelo slirp abre um socket ICMP no host que vive ~3 s e, enquanto vive, as novas conexões TCP do guest não completam (macOS, 2026-09-11: 6 SYN sem SYN-ACK com o DHCP a ser respondido normalmente); depois dela não há mais TCP. Captura pcap em `build/logs/net.pcap` (`--net-dump`; **só fiável quando o QEMU sai por si** — morto por timeout, o filtro não descarrega o buffer e o ficheiro fica nos primeiros KB); parsers e máquina de estados com suíte no host (`cargo test -p nexo-netstack`). Os auxiliares do host vivem até o QEMU sair (evento `parar`), não até um prazo: com prazo, uma primeira tentativa perdida num travamento do OVMF esgotava-os e a repetição ficava à espera de uma conexão que ninguém faria |
...
| Caça a intermitente | `tools/nexo-repro <cenario> <n>` | repete o cenário n vezes e conta os desfechos; os logs das falhas ficam preservados. Existe porque uma falha de uma em dez não se estuda esperando — e porque "não reproduziu" só tem significado com um denominador |
| Bateria por bloco | `make validar` | lint + imagem + os 12 cenários, **parando no primeiro erro**. É o alvo a usar antes de cada commit: encadear os passos à mão e ler só o fim da saída já deixou passar dois lints vermelhos para o `main` |
