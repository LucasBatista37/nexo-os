...
| Rede (QEMU) | `tools/test-qemu --scenario net` | `run-qemu --net` (slirp): DHCP completo (DISCOVER→OFFER→REQUEST→ACK, lease com máscara/gw/DNS), ARP resolve o gateway do lease e um ICMP echo request recebe o echo reply (ident/seq/checksum); parsers com fuzz-lite no host (`cargo test -p nexo-netstack`) |
