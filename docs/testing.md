...
| Rede (QEMU) | `tools/test-qemu --scenario net` | `run-qemu --net` (slirp): ARP resolve o gateway 10.0.2.2 e um ICMP echo request (montado com `nexo-netstack`) recebe o echo reply com ident/seq/checksum corretos; parsers com fuzz-lite no host (`cargo test -p nexo-netstack`) |
