...
| cliente → `vfs` | **tipado**: o mesmo `nexo.fs` v1.2, roteado por prefixo do namespace da instância: `/disk` → `fs`, `/boot` → `espfs` (só leitura; escrita → erro 13), `/tmp` → ramfs interno (16 arquivos × 16 KiB, volátil, por instância); inodes carregam a montagem nos bits 28..30 | um `vfs` por cliente = namespace por processo; handles: 0 `fs`, 1 `espfs`, 2 cliente; argumento = máscara de montagens |
...
| cliente → `fs` | **tipado**: `nexo.fs` v1.2 (`idl/fs.idl`; métodos 1–10: stat/create/mkdir/unlink/read/write/list/sync/info/truncate); erros remotos 1–11 = `nexofs::FsError::code()`, 255 malformado | um cliente por servidor (handle 1); ADR-0016 |
