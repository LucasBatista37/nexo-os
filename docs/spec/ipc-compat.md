...
A IDL existe: `idl/*.idl` → `tools/idlgen` (`make idl`) → `abi/proto` (`nexo-proto`, cabeçalho NXIP do §2, structs com encode/decode, enum de pedidos, erros remotos tipados; testes de layout, ida-e-volta, compatibilidade N±1 e fuzz-lite no crate). O CI falha se os módulos gerados estiverem defasados (`make idl-check`). **Migrados:** `nexo.rng`, `nexo.block` (`read`/`write`/`capacity`/`identity`), `nexo.console` (`read`/`write`) e `nexo.input` (`poll`) — todos v1.0, servidores e clientes. Continuam crus: `nexo.fs`, `nexo.esp`, `nexo.svcmgr`/`nexo.service` (echo) e as mensagens do `devmgr` ao cliente. Os demais protocolos abaixo continuam crus e serão migrados um a um:
