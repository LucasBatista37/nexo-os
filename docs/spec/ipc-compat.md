...
| cliente → `blockdev` (`nexo.block` v0) | pedido `[op u8][pad 3][setor u64][n u32][dados n×512 se escrita]`, `op` 0 = ler, 1 = escrever, 2 = capacidade (resposta `[0][setores u64]`), `n` ≤ 7 · resposta `[status u8][dados n×512 se leitura]`; status 0 ok, 1 pedido curto, 2 fora da capacidade/`n` inválido, 3 dados insuficientes, `0x1x` erro do dispositivo | um canal por cliente (handle 1 do driver); substituído por `nexo.block` tipado |
...
| `devmgr` → cliente | `fs`+handle (canal do servidor de arquivos), `rng`+handle (canal do `rngdev`), `done` | canal do handle 1 do `devmgr`; drivers recebem `[concessão, canal]` como handles iniciais |
