...
Gerado por `tools/roadmap-status` a partir de `PLANO_MESTRE_SISTEMA_OPERACIONAL.md` em 2026-08-30 (commit `eee583d`). Legenda: ✅ concluído · 🟡 parcial · ⬜ pendente · ⛔ bloqueado. Percentual = (concluídos + ½ parciais) / total.

**Total de itens do plano:** 535 — ✅ 150 · 🟡 51 · ⬜ 334 · ⛔ 0 → **33% do caminho até a 1.0** (ponderado por item, não por esforço: as fases restantes são muito maiores).
...
| Fase 5 — Gráficos, entrada e shell próprio (anos 3–5) | ⬜ não iniciado | 2 | 8 | 15 | 24% | 12–18 meses | renderer 2D, compositor, entrada, janelas, toolkit, temas, login/sessão, Contextos, Central de Ações, Faixa de Atividades, acessibilidade, testes de usabilidade |
...
### Fase 5 — Gráficos, entrada e shell próprio (anos 3–5) — 24% (2 ✅, 8 🟡, 15 ⬜)
...
| 🟡 | integrar mouse e teclado pelo serviço de entrada | o compositor `wm` tem uma **fonte de entrada** (`set_input`, canal de eventos evdev crus): move o ponteiro (EV_ABS) e reage ao clique (BTN_LEFT); auto-teste `user_wm_input` com eventos sintéticos. Falta ligar o serviço `inputdev` real a esse canal e tratar teclado (entrega de teclas à janela em foco) |
| 🟡 | implementar foco, atalhos e captura segura | foco por clique: o clique traz a superfície sob o ponteiro para a frente (`user_wm_input`). Atalhos globais, entrega de teclado à janela em foco e captura segura (grab) pendentes |
