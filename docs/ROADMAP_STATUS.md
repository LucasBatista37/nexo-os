...
Gerado por `tools/roadmap-status` a partir de `PLANO_MESTRE_SISTEMA_OPERACIONAL.md` em 2026-08-30 (commit `a3b2c6d`). Legenda: ✅ concluído · 🟡 parcial · ⬜ pendente · ⛔ bloqueado. Percentual = (concluídos + ½ parciais) / total.

**Total de itens do plano:** 535 — ✅ 147 · 🟡 33 · ⬜ 355 · ⛔ 0 → **31% do caminho até a 1.0** (ponderado por item, não por esforço: as fases restantes são muito maiores).
...
| Fase 4 — Rede e serviços básicos (anos 3–4) | ⬜ não iniciado | 1 | 5 | 13 | 18% | 6–12 meses | VirtIO net, Ethernet/ARP/IPv4/ICMP/UDP/TCP/DHCP/DNS, sockets, IPv6, firewall, TLS portado, HTTP, fuzzing de rede |
...
### Fase 4 — Rede e serviços básicos (anos 3–4) — 18% (1 ✅, 5 🟡, 13 ⬜)
