...
Gerado por `tools/roadmap-status` a partir de `PLANO_MESTRE_SISTEMA_OPERACIONAL.md` em 2026-09-02 (commit `52a56c1`). Legenda: ✅ concluído · 🟡 parcial · ⬜ pendente · ⛔ bloqueado. Percentual = (concluídos + ½ parciais) / total.
...
| 🟡 | implementar atualização atômica e health check pós-boot | o **health check pós-boot** existe: `services/upd`, com o canal `nexo.block` do disco de boot real (servido pelo `ahcidev` no SATA integrado), localiza `\nexo\slots.bin` (GPT → FAT, `nexo-fat::first_sector_lba`) e "confirma" marca o slot arrancado como saudável (sucesso = 1, tentativas repostas) gravando o setor de volta — auto-teste `user_slots` (93º) confere relendo do disco. A atualização atômica da imagem (escrever kernel/initrd novos no slot inativo e marcá-lo pendente) pende — exige escrita FAT |
