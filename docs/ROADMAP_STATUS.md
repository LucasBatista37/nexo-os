...
Gerado por `tools/roadmap-status` a partir de `PLANO_MESTRE_SISTEMA_OPERACIONAL.md` em 2026-09-03 (commit `472831d`). Legenda: ✅ concluído · 🟡 parcial · ⬜ pendente · ⛔ bloqueado. Percentual = (concluídos + ½ parciais) / total.
...
### Fase 6 — Plataforma de aplicativos e desktop essencial (anos 4–6) — 56% (7 ✅, 14 🟡, 4 ⬜)
...
| 🟡 | publicar headers e toolchain C/C++ | os **headers** existem: `abi/c/nexo.h` (wrappers inline da convenção de syscall) + `abi/c/nexo_syscalls.h` **gerado da fonte Rust** (`tools/nexo-cheaders` — nunca desatualiza), e o primeiro processo em C roda: `examples/c/hello.c` freestanding (clang + rust-lld, `tools/build-c-demo`), empacotado no initrd e provado pelo auto-teste `user_c_hello` (90º). A **libc mínima nasceu** (`sdk/libc`: `string.h` — memcpy/memset/memcmp/strlen/strcmp/strncmp — e `puts` via log do kernel; ADR-0014 passo 2), com o `hello.c` reescrito por cima exercitando cada função sob asserts. Lição de ABI descoberta no caminho: **processos de usuário ainda não têm SSE habilitado** — o toolchain C agora compila com `-mno-sse/-msoft-float` como o target Rust já fazia (a -O2 o clang vetorizava o memcpy e o processo morria com #UD). A restrição foi então removida na raiz: **FPU/SSE habilitado para usuário** (CR4.OSFXSR por CPU + FXSAVE/FXRSTOR por thread com estado limpo no spawn — auto-teste `user_sse`, 97º, prova o isolamento com dois processos XMM em paralelo); o SSE é opt-in por programa. E o **heap**: `malloc`/`free`/`calloc`/`realloc` sobre os objetos de memória do kernel (arenas de 256 KiB, primeira adequação com divisão; magic guards abortam free inválido) — um programa C no Nexo tem strings, saída e memória dinâmica. E o **C++ freestanding mínimo**: runtime na libc (new/delete sobre o malloc, `__cxa_pure_virtual`, construtores globais via `.init_array`), headers com guards `extern "C"`, e o primeiro processo C++ rodando — construtor global, despacho virtual por vtable e new/delete provados (auto-teste `user_cpp`, 98º; sem exceptions/RTTI, o perfil freestanding). E o **empacotamento**: `make toolchain` (`tools/nexo-toolchain`) monta o sysroot — headers da ABI e da libc, `libnexo.a`, wrappers `nexo-cc`/`nexo-c++` com as flags do alvo e README — gera o tarball e **se auto-testa** compilando um programa de fora usando apenas o pacote. Pende stdio de verdade (fd/arquivos) |
